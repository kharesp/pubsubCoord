...
def broker_cpumem_vs_time(test_dir,broker_type):
  ...
  for i in range(num_rbs):
    ...
    rb_cpu_vs_time.xaxis.set_major_locator(md.SecondLocator(interval=10))
    ...
    plt.xticks(rotation=20)
    rb_mem_vs_time.xaxis.set_major_locator(md.SecondLocator(interval=10))
...
def per_sample_latency(test_dir,num_topics):
  ...
  legend=[]
  for topic,files in sorted(topic_files_map.iteritems()):
...
def summary_statistics(test_dir,num_topics):
  ...
  with open('%s/summary.csv'%(test_dir),'w') as out:
    out.write('topic,#subscribers,mean latency(ms),min latency(ms),\
