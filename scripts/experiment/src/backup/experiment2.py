...
class Experiment(object):
  ...
  def kill_existing_processes(self):
    ...
    command_string='cd %s && ansible-playbook playbooks/experiment/kill.yml -f 8 --limit %s\
      --extra-vars="pattern=Broker,rtirouting,Monitor"'%(metadata.ansible,self.conf.brokers)
  ...
  def clean_logs(self):
    command_string='cd %s && ansible-playbook playbooks/experiment/clean.yml -f 8 --limit %s'\
      %(metadata.ansible,self.conf.hosts)
    subprocess.check_call(['bash','-c',command_string])

  def setup_infrastructure(self):
    #ensure netem rules are set on clients
    command_string='cd %s && ansible-playbook playbooks/experiment/netem_cli.yml -f 8 --limit %s'%\
      (metadata.ansible,','.join(self.conf.clients))
    subprocess.check_call(['bash','-c',command_string])

    #ensure netem rules are set on rbs 
    command_string='cd %s && ansible-playbook playbooks/experiment/netem_rb.yml -f 8 --limit %s'%\
      (metadata.ansible,','.join(self.conf.rbs))
    subprocess.check_call(['bash','-c',command_string])

    #start routing service on all brokers
    command_string='cd %s && ansible-playbook playbooks/experiment/rs.yml -f 8 --limit %s'%\
      (metadata.ansible,self.conf.brokers)
    subprocess.check_call(['bash','-c',command_string])

    #start EdgeBroker on ebs
    command_string='cd %s && ansible-playbook playbooks/experiment/eb.yml -f 8 --limit %s\
      --extra-vars="emulated_broker=%d"'%\
      (metadata.ansible,','.join(self.conf.ebs),1)
    subprocess.check_call(['bash','-c',command_string])

    #start monitoring process on ebs
    command_string='cd %s && ansible-playbook playbooks/experiment/monitoring.yml -f 8 --limit %s\
      --extra-vars="broker_type=eb run_id=%s"'%\
      (metadata.ansible,','.join(self.conf.ebs),self.run_id)
    subprocess.check_call(['bash','-c',command_string])

    #start RoutingBroker on rbs
    command_string='cd %s && ansible-playbook playbooks/experiment/rb.yml -f 8 --limit %s\
  ...
  def start_subscribers(self):
    ...
    for i in range(num):
      ...
      subprocess.check_call(['python','src/start_subscribers.py',rep,str(self.conf.sample_count),self.run_id])
    ...
    #    (metadata.ansible,host,str(topic_count_map).replace(" ",""),self.conf.sample_count,self.run_id)
  ...
  def start_publishers(self):
    ...
    for i in range(num):
      ...
      rep=json.dumps(pairs)
      subprocess.check_call(['python','src/start_publishers.py',rep,str(self.conf.sample_count),str(self.conf.sleep_interval),self.run_id])
  ...
  def collect_logs(self):
    ...
    command_string='cd %s && ansible-playbook playbooks/experiment/copy.yml -f 8 --limit %s\
